 ...
 * @param cb
 */
FileUtil.ls = function(path, cb, isRecursive){
	var self = this;
	var list = [];
	fs.readdir(path, function(err, files){
		...
		{	
			loop(files, 5, function(i, value, next){
				self.stat(pathUtil.join(path, value), function(stats){
					if(stats) 
					{
						...
						{
							self.ls(stats.path, function(err, sublist){
...
FileUtil.lsSync = function(path, isRecursive){
	var list = [];
	try{
...
FileUtil.copy = function(orgPath, destPath, cb) {
	var copy = function(e){
		fse.copy(orgPath, destPath, function(err){
			if(cb) cb(err);
		});
	};
